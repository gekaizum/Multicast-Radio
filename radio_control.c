...
void* station_listner(){//playing music
	FILE *fp;
	fp=popen("play -t mp3 -> /dev/null 2>&1","w");//open pipe to app that playing music
	int udp_socket;
	int reading;
	...
	while(stop_flag==0){//end of programm condition
		if(playing_station!=current_station){//station was changed by user, lets reconnect to new station
			...
			if((last_multicast_address+new_current_station)>255){//overflow in last octet, need to change previous octet of ip address
				...
				if(second_multicast_address>255){//overflow in secont octet
